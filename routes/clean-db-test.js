...
router.get(('/'), async function (req, res, next) {
    ...
    try {
        ...
        if (insertUnit.length !== 2) {
            throw new Error('Couldn\'t insert values into db_test.unit');
        }
        ...
        if (insertProduct.length !== 2) {
            throw new Error('Couldn\'t insert values into db_test.product');
        }
        ...
        if (insertLocation.length !== 2) {
            throw new Error('Couldn\'t insert values into db_test.location');
        }
        ...
        if (insertStock.length !== 2) {
            throw new Error('Couldn\'t insert values into db_test.stock');
        }

        await conn.commit();
        res.status(200).json(new ApiResult(200, 'OK', 1, []));
    } catch (e) {
        logger.error('clean-db-test: Error cleaning database db_test: ' + e);
        await conn.rollback();
        logger.info('clean-db-test: Transaction rolled back');
        const error = new ApiError('REG01', e.message, '', '');
        res.status(500).json(new ApiResult(500, 'Error cleaning database db_test', 1, [error]));
    }
});
